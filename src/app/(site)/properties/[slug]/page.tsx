    ...
    <>
      ...
      <div className="border-b border-border bg-ink-50">
        <div className="container-page flex items-center gap-1.5 py-3 text-xs text-muted-foreground">
          ...
          <Link href={ROUTES.PROPERTIES} className="hover:text-foreground">Properties</Link>
          <ChevronRight className="h-3 w-3" />
          <span className="max-w-48 line-clamp-1 font-medium text-foreground">{p.title}</span>
        </div>
      </div>
          ...
          <div className="space-y-6">
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="mt-3 text-3xl font-semibold text-brand-700">{displayPrice}</p>
                ...
                {p.priceNegotiable && (
                  <p className="mt-2 text-sm text-ink-500">Price is negotiable</p>
                )}
              </div>
            </div>
            ...
            <div className="grid gap-2 sm:grid-cols-3">
              <div className="surface-card bg-brand-aurora p-4 sm:col-span-2">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-700">Listing note</p>
                <p className="mt-3 text-sm leading-relaxed text-ink-700">
                  Verified cues, compact specs, and seller actions stay visible before user scrolls into long description.
                </p>
              </div>
              <div className="surface-card p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-ink-500">Views</p>
                <p className="mt-3 text-2xl font-semibold text-ink-900">{p.viewCount}</p>
              </div>
            </div>
          </div>
              ...
              <div className="relative h-[25rem]">
                ...
                <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4">
                  <div className="max-w-md text-white">
                    <p className="text-[0.68rem] uppercase tracking-[0.18em] text-white/70">{p.type}</p>
                    <p className="mt-2 text-lg font-semibold">{p.area}, {p.district}</p>
                  </div>
                  <div className="flex gap-2">
                    <SaveButton propertyId={p.id} className="h-10 w-10 bg-white/92 text-ink-700 hover:bg-white" />
                    <CompareButton property={p} className="h-10 w-10 bg-white/92 text-ink-700 hover:bg-white" />
                  </div>
                </div>
              </div>
            </div>
            {p.images.slice(1, 4).map((img, i) => (
              <div key={img} className={`surface-card relative overflow-hidden ${i === 0 ? "sm:col-span-1" : ""} h-36`}>
                <Image src={img} alt={`${p.title} ${i + 2}`} fill sizes="(max-width: 1024px) 33vw, 15vw" className="object-cover" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 grid gap-10 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {specCards.map(({ icon: Icon, label, value }) => (
                <div key={label} className="surface-card p-4 text-center">
                  ...
                  <p className="mt-2 text-xs uppercase tracking-[0.16em] text-ink-500">{label}</p>
              ...
              <div className="surface-card p-6">
                ...
                <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {p.facilities.map((facility) => (
                    <div key={facility} className="flex items-center gap-3 rounded-[1rem] bg-ink-50 px-4 py-3 text-sm text-ink-700">
            ...
            <div className="surface-card p-6">
              ...
              <div className="mt-5 h-56 rounded-[1.5rem] border border-border bg-brand-aurora flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-sm font-medium text-ink-800">{p.area}, {p.district}</p>
                  <p className="mt-1 text-xs uppercase tracking-[0.16em] text-ink-500">Map placeholder</p>
                </div>
              </div>
            </div>
            ...
            {similar.length > 0 && (
              <div>
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs uppercase tracking-[0.18em] text-brand-700">More like this</p>
                    <h2 className="mt-2 text-h3">Similar Properties</h2>
                  </div>
                  <Link href={ROUTES.PROPERTIES}>
                    <Button variant="outline" className="rounded-full border-brand-200 text-brand-700 hover:bg-brand-50">
                      Browse More
          ...
          <div className="space-y-4 lg:sticky lg:top-24 lg:self-start">
            <div className="surface-card bg-brand-aurora p-5">
              <p className="text-xs uppercase tracking-[0.18em] text-brand-700">Price snapshot</p>
              <p className="mt-3 text-3xl font-semibold text-brand-700">{displayPrice}</p>
              <p className="mt-2 text-sm text-ink-500">
                {p.purpose === "rent" ? "Monthly rent estimate." : "Purchase-ready starting point."}
              </p>
            </div>

            <div className="surface-card p-5">
              <h3 className="font-sans text-lg font-semibold text-ink-900">Contact Seller</h3>
              <div className="mt-4 flex items-center gap-3">
                {p.sellerAvatar && (
                  <Image src={p.sellerAvatar} alt={p.sellerName} width={44} height={44} className="rounded-full" />
                )}
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-ink-900">{p.sellerName}</p>
                  <p className="text-xs text-ink-500">{p.sellerVerified ? "Verified Seller" : "Independent Seller"}</p>
                </div>
              </div>
              ...
              <div className="mt-5 rounded-[1.25rem] border border-border bg-ink-50 p-4">
                <p className="text-sm leading-relaxed text-ink-600">
                  Need neutral help before talking to seller? Book consultation with Alivia team.
                </p>
